...
public abstract class AbstractSingleSourceShortestPathAlgorithm<Node> {
    ...
    // This method allows us to hide the constructor of the ShortestPathTree
    // from public audience.
    protected ShortestPathTree<Node>
            constructShortestPathTree(
                    Map<Node, Node> parents,
                    Map<Node, Double> distances,
                    Node sourceNode,
                    DoubleWeightFunction<Node> weightFunction) {
        return new ShortestPathTree<>(parents, distances, sourceNode);
    }
}
